...
(function(_win) {
    ...
    Intro = {
        ...
        tpl_pop: function(id) {
            return '<div id="' + id + '" class="sub-win">' +
        ...
        init: function() {
            ...
            for (var j = 0; j < 3; j++) {
                document.getElementById('Intro-' + j).querySelector('.close').addEventListener(
                    'click',
                    function() {
                        document.getElementById(this.getAttribute('data-toggle')).style.display = 'none';
                    }, false
                );
            }
        },
        ...
        callbackFunc: function(e) {
            if (e.target.id.substring(0, 4) == 'btn-') {
                ...
                switch (e.target.id) {
                    ...
                    case 'btn-1':
                        woh.runtime.init();
                        setTimeout('woh.gameScript.continueExec()', 300);
                        break;
                        //case 'btn-2':document.getElementById('Intro-0').style.display='block';break;
                    case 'btn-2':
                        woh.runtime.init();
                        woh.runtime.stageTag = 'drama_Infinity';
                        setTimeout('woh.gameScript.continueExec()', 300);
                        break;
                    case 'btn-3':
                        document.getElementById('Intro-1').style.display = 'block';
                        break;
                    case 'btn-4':
                        document.getElementById('Intro-2').style.display = 'block';
                        break;
                    default:
                        break;
                }
    ...
    var roleManager = _win.roleManager = {
            ...
            renderItems: function() {
                ...
                for (var i in itemdatas) {
                    itemlattics[i].innerHTML = "<img width='77' height='77' src='" + woh.item_data[itemdatas[i][0]][itemdatas[i][1]]['icon'] + "' draggable='false'" + "datatype='" + woh.runtime.packageItems[i][0] + "' datatag='" + woh.runtime.packageItems[i][1] + "' for='" + woh.item_data[itemdatas[i][0]][itemdatas[i][1]]['for'] + "'/>";
                }
            },
            init: function(data) {
        ...
        initData: function(result) {
            ...
            result.roledata.forEach(function(role) {
                ...
                items.push("<li class='exp-info'>" +
                    ...
                    "<p class='lv-info'>Lv:" + role.lv + "</p>" + "</li>");
            });
            //渲染等级数据
